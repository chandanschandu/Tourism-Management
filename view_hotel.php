<?php 
if(isset($_GET['id'])){
    $hotels = $conn->query("SELECT * FROM `hotels` where md5(id) = '{$_GET['id']}'");
    if($hotels->num_rows > 0){
        foreach($hotels->fetch_assoc() as $k => $v){
            $$k = $v;
        }
    }
$review = $conn->query("SELECT r.*,concat(firstname,' ',lastname) as name FROM `rate_review1` r inner join users u on r.user_id = u.id where r.hotel_id='{$id}' order by unix_timestamp(r.date_created) desc ");
            ...
            <div class="col-md-5">
                ...
                <div class="w-100">
                    ...
                    <div class="w-100 d-flex justify-content-between">
                        ...
                        <button class="btn btn-flat btn-warning" type="button" id="hotelbook">Book Now</button>
                    </div>
                </div>
            </div>
            <div class="col-md-7">
                ...
                <hr class="border-warning">
                <small class='text-muted'>Location: <?php echo $hotel_location ?></small>
                <h4>Details</h4>
    ...
    $(function(){
        $('#hotelbook').click(function(){
            if("<?php echo $_settings->userdata('id') ?>" > 0)
                uni_modal("Book Info","hotelbook_form.php?hotel_id=<?php echo $id ?>");
            else

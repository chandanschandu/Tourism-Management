<?php 
if(isset($_GET['id'])){
    $vehicles = $conn->query("SELECT * FROM `vehicles` where md5(id) = '{$_GET['id']}'");
    if($vehicles->num_rows > 0){
        foreach($vehicles->fetch_assoc() as $k => $v){
            $$k = $v;
        }
    }
$review = $conn->query("SELECT r.*,concat(firstname,' ',lastname) as name FROM `rate_review2` r inner join users u on r.user_id = u.id where r.vehicle_id='{$id}' order by unix_timestamp(r.date_created) desc ");
...
if(is_dir(base_app.'uploads/vehicle_'.$id)){
    $ofile = scandir(base_app.'uploads/vehicle_'.$id);
    foreach($ofile as $img){
        if(in_array($img,array('.','..')))
        continue;
        $files[] = validate_image('uploads/vehicle_'.$id.'/'.$img);
    }
}
}
?>
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-7">
                ...
                <hr class="border-warning">
                <small class='text-muted'>Location: <?php echo $vehicle_location ?></small>
                <h4>Details</h4>
    ...
    $(function(){
        $('#book').click(function(){
            if("<?php echo $_settings->userdata('id') ?>" > 0)
                uni_modal("Book Info","vehiclebook_form.php?vehicle_id=<?php echo $id ?>");
            else

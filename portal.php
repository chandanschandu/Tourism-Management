...
<section class="page-section bg-dark" id="home">
	...
	<div class="row">
		<?php
		$hotels = $conn->query("SELECT * FROM `hotels` order by rand() limit 3");
	...
	<div class="d-flex w-100 justify-content-end">
		<a href="./?page=hotels" class="btn btn-flat btn-warning mr-4">Explore hotels <i class="fa fa-arrow-right"></i></a>
	</div>
	</div>
</section>
<!-- vehicle -->
<section class="page-section bg-dark" id="home">
	...
	<div class="row">
		<?php
		$vehicles = $conn->query("SELECT * FROM `vehicles` order by rand() limit 3");
			while($row = $vehicles->fetch_assoc() ):
				$cover='';
				if(is_dir(base_app.'uploads/vehicle_'.$row['id'])){
					$img = scandir(base_app.'uploads/vehicle_'.$row['id']);
					$k = array_search('.',$img);
					if($k !== false)
						unset($img[$k]);
					$k = array_search('..',$img);
					if($k !== false)
						unset($img[$k]);
					$cover = isset($img[2]) ? 'uploads/vehicle_'.$row['id'].'/'.$img[2] : "";
				}
				$row['description'] = strip_tags(stripslashes(html_entity_decode($row['description'])));

				$review = $conn->query("SELECT * FROM `rate_review2` where vehicle_id='{$row['id']}'");
				$review_count =$review->num_rows;
				$rate = 0;
				while($r= $review->fetch_assoc()){
					$rate += $r['rate'];
		...
		?>
			<div class="col-md-4 p-4 ">
				<div class="card w-100 rounded-0">
    				...
    				<p class="card-text truncate"><?php echo $row['description'] ?></p>
					<div class="w-100 d-flex justify-content-end">
						<a href="./?page=view_vehicle&id=<?php echo md5($row['id']) ?>" class="btn btn-sm btn-flat btn-warning">View vehicle <i class="fa fa-arrow-right"></i></a>
					</div>
					</div>
				</div>
			</div>
	...
	<div class="d-flex w-100 justify-content-end">
		<a href="./?page=vehicles" class="btn btn-flat btn-warning mr-4">Explore vehicles <i class="fa fa-arrow-right"></i></a>
	</div>
	</div>
</section>
...
.services .box-container{
  ...
  h2{font-size:2.5rem;
...
</head><body><h2 class="text-center">SERVICES</h2><div class="services"> <div class="box-container">
    
            <div class="box">
                <i class="fas fa-hotel"></i>
                <h3>affordable hotels, food and drinks</h3>
                <p>we provide you with the world class features in our hotel with healthy,protienious food and with best quality service<br></p></div>
            <div class="box">
                <i class="fas fa-utensils"></i>
                <h3>affordable vehicles</h3>
                <p>we provide best trasport experience to our beloved costemers to make it easy to book vehiclers though us and with best service</p>
            </div>
            ...
            <div class="box">
               ...
               <h3>packages</h3>
                <p>we provide read to go packages to our travelers to make their trip easier and mmemorable</p>
            </div>
...
<section class="page-section" id="contact">
	<div class="container">
		<div class="text-center">
			...
			<h3 class="section-subheading text-muted">Send us a message for enquiries.</h3>
		</div>
		<!-- * * * * * * * * * * * * * * *-->

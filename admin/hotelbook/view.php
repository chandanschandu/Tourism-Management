...
include '../../config.php';
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT b.*,p.title,concat(u.firstname,' ',u.lastname) as name FROM hotelbook_list b inner join `hotels` p on p.id = b.hotel_id inner join users u on u.id = b.user_id where b.id = '{$_GET['id']}' ");

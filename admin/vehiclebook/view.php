...
include '../../config.php';
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT b.*,p.title,concat(u.firstname,' ',u.lastname) as name FROM vehiclebook_list b inner join `vehicles` p on p.id = b.vehicle_id inner join users u on u.id = b.user_id where b.id = '{$_GET['id']}' ");
...
<p><b>Schedule:</b> <?php echo date("F d, Y",strtotime("schedule1")) ?></p>
<form action="" id="vehiclebook-status">
...
<script>
    $(function(){
        $('#vehiclebook-status').submit(function(e){
            e.preventDefault();
            start_loader()
            $.ajax({
                url:_base_url_+"classes/Master.php?f=update_vehiclebook_status",
                method:"POST",
                data:$(this).serialize(),
                dataType:"json",
                error:err=>{
